...
## When you encounter the population sequentually, $N$ not known
...
The following results follow immediately from the previous slide:
...
> For example, $P(y_1 = y) = \frac{1}{N}$. But $P(y_1 = y\,|\, y_2) = \frac{1}{N-1}$ for $y_1 \ne y$. 
...
> So we have our unknown $\tau$ and $\mu$ (and $\sigma^2$) and we have a sample $\flist{y}{n}$ whose properties we have established. 

> How shall we use the sample to estimate the unknown paramaters? We'll go with an obvious choices for $\mu$ and $\tau$ which are: $$\hat\mu = \bar{y} = \frac{1}{n}\sum_{i=1}^n y_i \qquad \text{and} \qquad \hat\tau = N\hat\mu = N\bar{y} = \frac{N}{n}\sum_{i=1}^n y_i $$ 
